              ...
              <div className={`sidebar-section-container fields-section ${sidebarSections.fields ? '' : 'collapsed'}`}>
                ...
                <div className="sidebar-section-body">
                  ...
                  >
                    ...
                    <span>CUSTOM</span>
                    <span className="system-badge custom-badge-header" style={{ 
                      marginLeft: 'auto',
                      fontSize: '0.75rem'
                    }}>CUSTOM</span>
                  </div>
                  {fieldBrowsers.customFields && (
                    <div className="sidebar-nav-item-content">
                      <div className="custom-field-panel-form" style={{ padding: '12px' }}>
                        <div className="form-field">
                          <label>System</label>
                          <select
                            value={customFieldForm.system}
                            onChange={(e) => setCustomFieldForm({ ...customFieldForm, system: e.target.value })}
                          >
                            <option value="">Select...</option>
                            <option value="Centric">Centric</option>
                            <option value="Fulfil">Fulfil</option>
                        ...
                        <div className="form-field">
                          ...
                          <input
                            ...
                            value={customFieldForm.object}
                            onChange={(e) => setCustomFieldForm({ ...customFieldForm, object: e.target.value })}
                            placeholder="e.g. Product"
                          />
                          <datalist id="object-suggestions">
                            {customFieldForm.system && Array.from(new Set(
                              fieldsCatalog
                                .filter(f => f.system === customFieldForm.system)
                                .map(f => f.object)
                            )).sort().map(obj => (
                        ...
                        <div className="form-field">
                          ...
                          <input
                            type="text"
                            value={customFieldForm.field}
                            onChange={(e) => setCustomFieldForm({ ...customFieldForm, field: e.target.value })}
                        ...
                        <div className="form-field">
                          ...
                          <input
                            type="text"
                            value={customFieldForm.data_type}
                            onChange={(e) => setCustomFieldForm({ ...customFieldForm, data_type: e.target.value })}
                            placeholder="e.g. string, number, boolean"
                          />
                        </div>
                        <button
                          className="create-custom-field-btn"
                          onClick={customFieldForm.editingId ? handleSaveEditedCustomField : handleCreateCustomFieldFromPanel}
                          disabled={!customFieldForm.system || !customFieldForm.object || !customFieldForm.field}
                        >
                          {customFieldForm.editingId ? 'Update Field' : 'Create Field'}
                        </button>
                        {customFieldForm.editingId && (
                          <button
                            className="cancel-edit-btn"
                            onClick={() => setCustomFieldForm({ system: "", object: "", field: "", data_type: "string" })}
                            style={{ marginLeft: '8px', background: 'var(--bg-hover)', color: 'var(--text-muted)' }}
                          >
                            Cancel
                          </button>
                        )}
                      ...
                      <div className="custom-fields-list" style={{ padding: '12px', paddingTop: '0' }}>
                        <div className="custom-fields-list-header">Custom Fields ({allFields.filter(f => f.is_custom).length})</div>
                        {allFields.filter(f => f.is_custom).length === 0 ? (
                          <div className="empty-hint">No custom fields created yet</div>
                        ) : (
                          allFields.filter(f => f.is_custom).map(field => (
                            <div key={field.id} className="custom-field-list-item">
                              <div className="custom-field-info">
                                <div className="custom-field-name">{field.field}</div>
                              ...
                              <div className="custom-field-actions">
                                <button
                                  className="custom-field-action-btn"
                                ...
                                <button
                                  className="custom-field-action-btn delete"
